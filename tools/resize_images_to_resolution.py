...
def resize_images(
    ...
    cv2_interpolation = cv2.INTER_NEAREST_EXACT
    ...
    for filename in os.listdir(src_img_folder):
        ...
        if not filename.endswith(img_exts):
            ...
            shutil.copy(
                os.path.join(src_img_folder, filename),
                os.path.join(dst_img_folder, filename),
            )
            continue
        ...
        for max_resolution in max_resolutions:
            # Calculate max_pixels from max_resolution string
            max_pixels = int(max_resolution.split("x")[0]) * int(
                max_resolution.split("x")[1]
            )
            ...
            if current_pixels > max_pixels:
                ...
                # Resize image
                img = cv2.resize(
                    img, (new_width, new_height), interpolation=cv2_interpolation
                )
            else:
            ...
            new_height = (
                ...
                else new_height - new_height % divisible_by
            )
            new_width = (
                new_width
                if new_width % divisible_by == 0
                else new_width - new_width % divisible_by
            )
            ...
            new_filename = (
                base + "+" + max_resolution + (".png" if save_as_png else ".jpg")
            )
            ...
            logger.info(
                f"{proc} image: {filename} with size {img.shape[0]}x{img.shape[1]} as {new_filename}"
            )
        ...
        if copy_associated_files:
            ...
            for asoc_file in asoc_files:
                ...
                for max_resolution in max_resolutions:
                    ...
                    shutil.copy(
                        os.path.join(src_img_folder, asoc_file),
                        os.path.join(dst_img_folder, new_asoc_file),
                    )
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "src_img_folder",
        type=str,
        help="Source folder containing the images / 元画像のフォルダ",
    )
    parser.add_argument(
        "dst_img_folder",
        type=str,
    ...
    parser.add_argument(
        "--interpolation",
        type=str,
        choices=["area", "cubic", "lanczos4"],
        default="area",
        help="Interpolation method for resizing / リサイズ時の補完方法",
    )
    parser.add_argument(
        "--save_as_png", action="store_true", help="Save as png format / png形式で保存"
    )
    parser.add_argument(
